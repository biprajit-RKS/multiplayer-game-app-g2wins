...
var GameHost = function (server) {
    var self = {
        ...
        onClientRequestPrivateRoom: function (socket, requestInfo) {
            ...
            for (var i = 0; i < self.pendingBattles.length; i++) {
                var battle = self.pendingBattles[i];
                if (battle.id == requestInfo.pw && battle.roomSize == requestInfo.roomSize) {
        ...
        getJoinableRoom: function (requestInfo) {
            ...
            for (var i = 0, il = self.roomList.length; i < il; i++) {
                ...
                // Match rooms with same roomSize, roomType, and payAmount
                ...
                if (!room.isPrivate && !room.hasStarted && 
                    ...
                    found_room = room;
                    console.log('Found matching room:', room.id, 'roomType:', room.roomType, 'payAmount:', room.payAmount);
                    break;
                }
            }
            if (!found_room) {
                found_room = self.createRoom(requestInfo.roomSize);
                // Store roomType and payAmount in the room
                ...
                console.log('Created new room:', found_room.id, 'roomType:', found_room.roomType, 'payAmount:', found_room.payAmount);
            }
            return found_room;
        },
