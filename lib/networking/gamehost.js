...
var GameHost = function (server) {
  var self = {
    ...
    onClientRequestGameState: function (socket) {
      ...
      if (
        room.currentPlayerID == player.id &&
        room.lastEvent != network.Events.DICE_ROLLING
      ) {
        room.sendGameState(socket);
        room.checkAutoMode();
      } else {
    ...
    syncTime: function (socket, data) {
      ...
      if (!player) return;
      if (data.getTime) {
        if (player.state.pauseTime == null)
          player.state.pauseTime = player.joinedTime;
        ...
        } else player.state.pauseTime = Date.now();
      }
    },
    ...
    onClientRequestPrivateRoom: function (socket, requestInfo) {
      ...
      for (var i = 0; i < self.pendingBattles.length; i++) {
        var battle = self.pendingBattles[i];
        var payAmountMatch =
          !requestInfo.payAmount || battle.payAmount === requestInfo.payAmount;

        if (
          battle.id == requestInfo.pw &&
          battle.roomSize == requestInfo.roomSize &&
          payAmountMatch
        ) {
      ...
      if (!check) {
        ...
        console.log(
          "🔒 Created private battle:",
          initBattle.id,
          "Amount:",
          initBattle.payAmount,
        );
      }
      return false;
    },
    ...
    getJoinableRoom: function (requestInfo) {
      ...
      for (var i = 0, il = self.roomList.length; i < il; i++) {
        ...
        var roomTypeMatch =
          !requestInfo.roomType || room.roomType === requestInfo.roomType;
        var payAmountMatch =
          !requestInfo.payAmount || room.payAmount === requestInfo.payAmount;
...
var Room = function (host, id, roomSize) {
  ...
  self.activeAutoMode = function () {
    ...
    if (self.lastEvent == network.Events.DICE_ROLLING) {
      ...
      self.lastEvent == network.Events.NEXT_TURN ||
      self.lastEvent == network.Events.CHECK_AUTO ||
      self.lastEvent == network.Events.HORSE_MOVE ||
      self.lastEvent == network.Events.PLAYER_COMPLETE
    ) {
      // SEND DICE ROLLING
      var data = {
  ...
  self.handleEven = function (player, msg) {
    ...
    if (msg.event == network.Events.INACTIVE) {
      ...
      if (
        self.requestingList.length > 0 &&
        self.requestingList.length ==
          self.playerList.length - self.inactivePlayers
      ) {
    ...
    if (
      ...
      msg.event != network.Events.NEXT_TURN &&
      msg.event != network.Events.HORSE_MOVE
    ) {
      return;
    }
  ...
  self.nextTurn = function () {
    ...
    while (
      (self.playerList[newId] == null || self.playerList[newId].completed) &&
      newId != id
    ) {
      newId++;
      if (newId == self.playerList.length) newId = 0;
  ...
  self.checkHorseReady = function (playerId) {
    ...
    for (var i = 0; i < 4; i++) {
      ...
      if (
        self.gameState.hasOwnProperty(horseId) &&
        self.gameState[horseId] > 0
      ) {
        check = false;
        break;
      }
